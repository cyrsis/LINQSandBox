...
namespace DemoXCount
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                Console.WriteLine("姓名：{0} 訂單總價：{1}", item.Guest.Attribute("姓名").Value, item.Count.ToString("C"));
                foreach (var p in item.Guest.Elements("訂貨單"))
                    Console.WriteLine("{0} {1} {2}", p.Element("品名").Value, p.Element("單價").Value, p.Element("數量").Value);
                Console.WriteLine("----------------------------------------------");
            }
